...
Local preparation and hosted progress (Phase 7 remains partial): Claude R5-1–R5-4 addressed; Linux API and pinned Flutter Web builds verified; restricted operator retention and a finite hosted preflight added; dedicated Android release signing configured; native/rematch invitations support the shared Web URL. See [current evidence](docs/status.md) and [history](docs/phase-7-verification.md). Hosted permissions/TLS, application backup/isolated restore and real URLs are verified. The pause-write fix is deployed and migration history is reconciled. Measured ingress, iOS provisioning, full Auth recovery, privileged hosted retention, full matches and soak measurements remain open.
