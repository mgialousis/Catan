...
export function projectEffects(effects: readonly EngineEffect[], viewerPlayerId: string) {
  const activity = effects.filter((e): e is Extract<EngineEffect, { type: 'PUBLIC_ACTIVITY' }> => e.type === 'PUBLIC_ACTIVITY').map(e => ({ type: e.action, actorPlayerId: e.actorPlayerId, message: e.message, ...(e.subjectPlayerId === undefined ? {} : { subjectPlayerId: e.subjectPlayerId }), ...(e.resources === undefined ? {} : { resources: { ...e.resources } }), ...(e.give === undefined ? {} : { give: { ...e.give } }), ...(e.receive === undefined ? {} : { receive: { ...e.receive } }) }));
