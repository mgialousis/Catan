...
**Phase 7 is not complete:** the hosted API and Web client are live, but the latest fixes still need an explicit deployment. Migration-history reconciliation, measured ingress, hosted recovery, iOS provisioning and real-device/network acceptance remain open. Builds made with `config/release.example.json` or default Docker Web arguments have placeholder URLs and are packaging checks only. Keep `.local/signing` and `android/key.properties` backed up privately; they are excluded from Git.
